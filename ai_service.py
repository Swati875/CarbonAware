...
if IS_GEMINI_ENABLED:
    try:
        ...
        logger.error(f"Error initializing Gemini AI: {e}. Falling back to rule-based mock advisor.")
        model = None
# Smart rule-based advisor for mock fallback
def _generate_mock_advice(user_message: str, carbon_data: dict = None) -> str:
    ...
    # Transport questions
    if "transport" in message_lower or "car" in message_lower or "drive" in message_lower or "fly" in message_lower or "flight" in message_lower:
        return (
            f"{carbon_summary}"
    ...
    # Energy questions
    if "energy" in message_lower or "electricity" in message_lower or "solar" in message_lower or "power" in message_lower or "heat" in message_lower:
        return (
            f"{carbon_summary}"
    ...
    # Diet questions
    if "diet" in message_lower or "food" in message_lower or "meat" in message_lower or "vegan" in message_lower or "eat" in message_lower:
        return (
            f"{carbon_summary}"
            "🥗 **Diet & Food Tips:**\n\n"
    ...
    # Waste questions
    if "waste" in message_lower or "recycle" in message_lower or "plastic" in message_lower or "trash" in message_lower:
        return (
            f"{carbon_summary}"
    ...
    if "calculate" in message_lower or "footprint" in message_lower or "analyze" in message_lower or "my score" in message_lower:
        if highest_source:
            tips = {
...
async def get_coaching_response(user_message: str, carbon_data: dict = None) -> str:
    ...
    try:
        ...
        logger.error(f"Error calling Gemini API: {e}. Falling back to mock advice.")
        return _generate_mock_advice(user_message, carbon_data)
